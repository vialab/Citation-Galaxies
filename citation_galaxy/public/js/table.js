class Table {
    ...
    setHeaderNames(names) {
        if (names.length != this.columns) {
            console.error(`Incorrect number of names being passed into header. Expected ${this.columns} got ${names.length}`);
        }
    ...
    setColumnsContent(columnNumber, innerHtml) {
        if (columnNumber > this.columns || columnNumber < 0) {
            console.error(`Error setting column's content. Column supplied ${columnNumber} must be > 0 and < ${this.columns}`);
        }
    ...
    getCell(row, column) {
        if (!$("#" + this.name).length) {
            console.error("Table does not exist. Table must be appended to Dom before getCell is called");
        }
