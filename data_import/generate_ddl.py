...
prepopulate_templates = [
    ...
    {
        "name": "Article Search insert dummy",
        "drop": "drop table if exists insert_dummy cascade;",
        "sql": """
-- CREATE EXTENSION uint;
create table insert_dummy (
...
""",
        "templater": lambda sql: ('\n'.join( (sql.format(year,'{}'.format( (', '.join( ( f'cite_in_{col:02d} smallint' for col in c_100) ) ) )) for year in year_range) ) + '\n')
    },
    {
...
"""
    },
    {
        ...
        "sql": """create table article_text_{} (
    id              SERIAL PRIMARY KEY,
    pub_year		smallint,
    title 		    varchar,
    abstract		varchar,
...
BEGIN
	EXECUTE format('INSERT INTO article_text_%s (id, pub_year, title, abstract, general, sections) VALUES ($1, $2, $3, $4, $5, $6) on conflict on constraint article_text_%s_pkey do nothing;'::text, new.pub_year, new.pub_year) 
...
CREATE TABLE query_cache (
    ...
    {
        "name": "signalcategory",
        "drop": "drop table if exists signalcategory cascade;",
        "sql":"""
CREATE TABLE signalcategory (
    ...
    {
        "name": "signal",
        "drop": "drop table if exists signal cascade;",
        "sql":"""
CREATE TABLE signal (
	id serial primary key,
	signalcategoryid int4 not null references signalcategory (id),
