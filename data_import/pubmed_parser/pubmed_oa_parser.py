...
def parse_pubmed_paragraph(path, ref_replace='', all_paragraph=False):
    """
    ...
    for paragraph in paragraphs:
        ...
        # pos = len( paragraph_text[0] or '' )
        # for child in paragraph.getiterator():
        #     if child.tag == 'xref':
        #         # need the current position in the document to compare it against the total length
        #         # paragraph_text = [ ''.join(paragraph_text) ]

        #         cite_pos = pos + (len(child.attrib['rid']))/2
        #         percentile = math.floor( (cite_pos/orig_len)*100 )
        #         citation_distribution[percentile-1] += 1

        #         paragraph_text.append( ref_replace )
        #         pos += len(child.attrib['rid'])
        #         paragraph_text.append( child.text if len(child.text or '')>1 else '' )
        #         paragraph_text.append( child.tail if len(child.tail or '')>1 else '' )
        #     elif child.tag != 'sup':
        #     # else:
        #         paragraph_text.append( child.text or '' )
        #         paragraph_text.append( child.tail or '' )
        #         pos += (len(child.text or '') + len(child.tail or ''))
        # paragraph_text.append(paragraph.tail or '')

        # paragraph_text = ''.join(filter(None,paragraph_text))
        ...
        ref_ids = [ reference.attrib['rid'] for reference in paragraph.getchildren() if 'rid' in reference.attrib.keys() ]
        # ref_ids = list()
